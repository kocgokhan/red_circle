...
public class FollowNotification {
    ...
    public FollowNotification(JSONObject response, boolean isLogin) {
        try {
            ...
            this.notification_id = response.getString("follow_notif_id");
            if (isLogin) {
                this.sender_images = response.getString("sender_images");
                this.sendDate = response.getString("like_time");
                this.message = response.getString("message");
    ...
    public static ArrayList<FollowNotification> getDataFollow (ArrayList<FollowNotification> ary) {
        ArrayList<FollowNotification> productList = new ArrayList<FollowNotification>();

        for (int i = 0; i < ary.size(); i++) {
            FollowNotification temp = new FollowNotification();
