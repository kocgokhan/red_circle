...
public class LikeNotification {
    ...
    private String post_id,sender_user_id,receiver_user_id,like_notif_id;
    ...
    public LikeNotification(JSONObject response, boolean isLogin) {
        try {
            ...
            this.like_notif_id = response.getString("like_notif_id");
            if (isLogin) {
                this.sender_images = response.getString("sender_images");
                this.sendDate = response.getString("like_time");
                this.message = response.getString("message");
    ...
    public String getNotification_id() {
        return like_notif_id;
    }

    public void setNotification_id(String notification_id) {
        this.like_notif_id = notification_id;
    }
    ...
    public static ArrayList<LikeNotification> getDataLike (ArrayList<LikeNotification> ary) {
        ArrayList<LikeNotification> productList = new ArrayList<LikeNotification>();

        for (int i = 0; i < ary.size(); i++) {
            LikeNotification temp = new LikeNotification();
